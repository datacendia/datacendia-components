...
export const SecurityOverviewPage: React.FC = () => {
  ...
  const recentEvents = [
    { id: 1, type: 'login', user: 'john@acme.com', action: 'Successful login', location: 'New York, US', time: new Date(Date.now() - 300000) },
    { id: 2, type: 'access', user: 'sarah@acme.com', action: 'Accessed sensitive dataset', location: 'Chicago, US', time: new Date(Date.now() - 900000) },
    { id: 3, type: 'policy', user: 'mike@acme.com', action: 'Policy violation: export attempt', location: 'Boston, US', time: new Date(Date.now() - 1800000), isAlert: true },
    { id: 4, type: 'login', user: 'emily@acme.com', action: 'Failed login attempt (3x)', location: 'Unknown', time: new Date(Date.now() - 3600000), isAlert: true },
  ];
            ...
            <div className="grid grid-cols-3 gap-4">
              {metrics.slice(1).map((m) => (
                <div key={m.label} className="p-3 bg-neutral-50 rounded-lg">
                  ...
                  <p className={cn(
                    'text-xl font-bold',
                    m.status === 'good' ? 'text-success-main' :
                    m.status === 'warning' ? 'text-warning-main' : 'text-info-main'
                  )}>{m.value}</p>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-xl border border-neutral-200 p-6">
          ...
          <div className="space-y-3">
            {complianceStatus.map((item) => (
              <div key={item.framework} className="flex items-center justify-between p-3 bg-neutral-50 rounded-lg">
...
export const AccessControlPage: React.FC = () => {
  ...
  const accessPolicies = [
    ...
    { id: 3, name: 'PII Data Restriction', type: 'Data-based', subjects: 'All except HR', resources: 'PII Fields', effect: 'Deny' },
    { id: 4, name: 'Admin Full Access', type: 'Role-based', subjects: 'Admins', resources: 'All Resources', effect: 'Allow' },
    { id: 5, name: 'External Contractor', type: 'Time-based', subjects: 'Contractors', resources: 'Project Data', effect: 'Allow (9-5 EST)' },
  ];

  const recentRequests = [
    { id: 1, user: 'John Smith', resource: 'Financial Reports', status: 'pending', requestedAt: new Date(Date.now() - 3600000) },
    { id: 2, user: 'Sarah Chen', resource: 'Customer PII', status: 'approved', requestedAt: new Date(Date.now() - 86400000) },
    { id: 3, user: 'Mike Johnson', resource: 'Admin Console', status: 'denied', requestedAt: new Date(Date.now() - 172800000) },
  ];
              ...
              <tr key={policy.id} className="border-b border-neutral-100 hover:bg-neutral-50">
                ...
                <td className="px-4 py-4">
                  <span className={cn(
                    ...
                    policy.effect.includes('Allow') ? 'bg-success-light text-success-dark' : 'bg-error-light text-error-dark'
                  )}>
                    {policy.effect}
                  </span>
                </td>
...
export const AuditLogPage: React.FC = () => {
  ...
  const auditLogs = [
    { id: 1, timestamp: new Date(Date.now() - 60000), user: 'john@acme.com', action: 'LOGIN', resource: '-', ip: '192.168.1.1', status: 'success' },
    { id: 2, timestamp: new Date(Date.now() - 120000), user: 'john@acme.com', action: 'VIEW', resource: 'Dashboard', ip: '192.168.1.1', status: 'success' },
    { id: 3, timestamp: new Date(Date.now() - 180000), user: 'sarah@acme.com', action: 'EXPORT', resource: 'customers.csv', ip: '192.168.1.2', status: 'blocked' },
    { id: 4, timestamp: new Date(Date.now() - 300000), user: 'mike@acme.com', action: 'UPDATE', resource: 'User Settings', ip: '192.168.1.3', status: 'success' },
    { id: 5, timestamp: new Date(Date.now() - 600000), user: 'emily@acme.com', action: 'DELETE', resource: 'Report #123', ip: '192.168.1.4', status: 'success' },
    { id: 6, timestamp: new Date(Date.now() - 900000), user: 'tom@acme.com', action: 'LOGIN', resource: '-', ip: '10.0.0.1', status: 'failed' },
    { id: 7, timestamp: new Date(Date.now() - 1200000), user: 'tom@acme.com', action: 'LOGIN', resource: '-', ip: '10.0.0.1', status: 'failed' },
    { id: 8, timestamp: new Date(Date.now() - 1500000), user: 'tom@acme.com', action: 'LOGIN', resource: '-', ip: '10.0.0.1', status: 'failed' },
  ];
...
export const SecurityPoliciesPage: React.FC = () => {
  ...
  const policies = [
    ...
    { id: 3, name: 'Data Export Policy', status: 'active', description: 'Require approval for exports over 1000 records', lastUpdated: 'Sep 20, 2025' },
    { id: 4, name: 'IP Allowlist', status: 'draft', description: 'Restrict access to corporate IP ranges', lastUpdated: 'Nov 20, 2025' },
    { id: 5, name: 'MFA Enforcement', status: 'active', description: 'Require MFA for all users', lastUpdated: 'Aug 1, 2025' },
  ];
              ...
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className={cn(
                    ...
                    policy.status === 'active' ? 'bg-success-light text-success-dark' : 'bg-neutral-100 text-neutral-600'
                  )}>
                    {policy.status}
                  </span>
                </div>
