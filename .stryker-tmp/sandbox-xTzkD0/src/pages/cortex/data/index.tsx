...
export const DataSourcesPage: React.FC = () => {
  ...
  const sources = [
    { id: 1, name: 'Salesforce CRM', type: 'CRM', icon: '☁️', status: 'connected', lastSync: new Date(Date.now() - 300000), records: 125000, growth: 12 },
    { id: 2, name: 'Snowflake DW', type: 'Data Warehouse', icon: '❄️', status: 'connected', lastSync: new Date(Date.now() - 1800000), records: 45000000, growth: 5 },
    { id: 3, name: 'SAP ERP', type: 'ERP', icon: '📊', status: 'connected', lastSync: new Date(Date.now() - 3600000), records: 8500000, growth: 3 },
    { id: 4, name: 'HubSpot', type: 'Marketing', icon: '🧡', status: 'syncing', lastSync: null, records: 250000, growth: 18 },
    { id: 5, name: 'Stripe', type: 'Payments', icon: '💳', status: 'connected', lastSync: new Date(Date.now() - 600000), records: 890000, growth: 8 },
    { id: 6, name: 'PostgreSQL (Legacy)', type: 'Database', icon: '🐘', status: 'error', lastSync: new Date(Date.now() - 86400000), records: 12000000, growth: -2 },
  ];
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Connected Sources', value: sources.filter(s => s.status === 'connected').length },
...
export const DataCatalogPage: React.FC = () => {
  ...
  const datasets = [
    { id: 1, name: 'customers', source: 'Salesforce', type: 'Table', columns: 45, rows: 125000, owner: 'Sales', tags: ['core', 'pii'], quality: 94 },
    { id: 2, name: 'orders', source: 'SAP ERP', type: 'Table', columns: 32, rows: 890000, owner: 'Finance', tags: ['core', 'transactional'], quality: 98 },
    { id: 3, name: 'products', source: 'PostgreSQL', type: 'Table', columns: 28, rows: 15000, owner: 'Product', tags: ['master'], quality: 87 },
    { id: 4, name: 'revenue_metrics', source: 'Snowflake', type: 'View', columns: 12, rows: null, owner: 'Finance', tags: ['kpi', 'aggregated'], quality: 96 },
    { id: 5, name: 'marketing_campaigns', source: 'HubSpot', type: 'Table', columns: 38, rows: 2500, owner: 'Marketing', tags: ['campaigns'], quality: 82 },
    { id: 6, name: 'employee_directory', source: 'Workday', type: 'Table', columns: 52, rows: 1200, owner: 'HR', tags: ['pii', 'sensitive'], quality: 91 },
  ];
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn('p-2 rounded-lg', viewMode === 'grid' ? 'bg-neutral-100' : 'hover:bg-neutral-50')}
          >
            ▦
          </button>
          <button
            onClick={() => setViewMode('list')}
            className={cn('p-2 rounded-lg', viewMode === 'list' ? 'bg-neutral-100' : 'hover:bg-neutral-50')}
          >
            ≡
          </button>
        </div>
          ...
          >
            <div className="flex items-start justify-between mb-3">
              ...
              <div className={cn(
                ...
                dataset.quality >= 90 ? 'bg-success-light text-success-dark' :
                dataset.quality >= 70 ? 'bg-warning-light text-warning-dark' :
...
export const DataQualityPage: React.FC = () => {
  ...
  const recentIssues = [
    { id: 1, severity: 'critical', dataset: 'customers', issue: 'Null values in email column', affected: 1234, detected: new Date(Date.now() - 3600000) },
    { id: 2, severity: 'warning', dataset: 'orders', issue: 'Date format inconsistency', affected: 567, detected: new Date(Date.now() - 7200000) },
    { id: 3, severity: 'warning', dataset: 'products', issue: 'Duplicate SKU values', affected: 23, detected: new Date(Date.now() - 14400000) },
    { id: 4, severity: 'info', dataset: 'marketing_campaigns', issue: 'Missing campaign end dates', affected: 45, detected: new Date(Date.now() - 28800000) },
  ];
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        {qualityDimensions.map((dim) => (
          <div key={dim.name} className="bg-white rounded-xl border border-neutral-200 p-4">
            ...
            <p className={cn(
              'text-sm font-medium',
              dim.trend === 'up' ? 'text-success-main' : dim.trend === 'down' ? 'text-error-main' : 'text-neutral-500'
            )}>
...
export const DataImportExportPage: React.FC = () => {
  const recentExports = [
    { id: 1, name: 'customers_backup.csv', size: 45600000, status: 'completed', date: new Date(Date.now() - 86400000) },
    { id: 2, name: 'orders_2025.parquet', size: 890000000, status: 'completed', date: new Date(Date.now() - 172800000) },
          ...
          {recentExports.map((exp) => (
            <div key={exp.id} className="flex items-center justify-between p-4 bg-neutral-50 rounded-lg">
              ...
              <div className="flex items-center gap-4">
                <span className={cn(
                  'px-2 py-1 rounded-full text-xs font-medium',
                  exp.status === 'completed' ? 'bg-success-light text-success-dark' : 'bg-warning-light text-warning-dark'
                )}>
                  {exp.status}
                </span>
