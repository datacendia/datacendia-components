...
const agents = [
  { code: 'quant', name: 'Quantitative Analyst', purpose: 'Financial modeling, derivatives pricing, risk quantification', model: 'qwq:32b' },
  { code: 'pm', name: 'Portfolio Manager', purpose: 'Asset allocation, investment strategy, rebalancing decisions', model: 'llama3.3:70b' },
  { code: 'cro-finance', name: 'Credit Risk Officer', purpose: 'Credit analysis, Basel compliance, counterparty risk', model: 'qwq:32b' },
  { code: 'treasury', name: 'Treasury Analyst', purpose: 'Cash management, FX exposure, liquidity planning', model: 'llama3.3:70b' },
];

const overlays = [
  { name: 'Fraud Detection Agent', function: 'Real-time transaction anomaly detection, pattern recognition', integrates: 'CendiaAegis™' },
  { name: 'SEC Compliance Radar', function: '10-K/10-Q deadline tracking, material event monitoring', integrates: 'CendiaPanopticon™' },
  { name: 'Basel Capital Calculator', function: 'Automated RWA computation, capital adequacy alerts', integrates: 'CendiaPredict™' },
  { name: 'AML Pattern Matcher', function: 'Suspicious activity detection, SAR workflow automation', integrates: 'CendiaGuard™' },
];

const compliance = ['SOX', 'Basel III/IV', 'GDPR', 'CCPA', 'GLBA', 'AML/BSA', 'CFPB', 'OCC', 'FDIC', 'Fed SR Letters', 'FINRA', 'SEC', 'CCAR/DFAST'];

const pricing = [
  { package: 'Banking Starter', price: '$120,000', includes: '8 Pillars + 4 Finance Agents', roi: '6 months' },
  { package: 'Banking Professional', price: '$800,000', includes: '+ Panopticon, Crucible, Aegis', roi: '4 months' },
  { package: 'Banking Enterprise', price: '$3,000,000', includes: '+ Full Guardian Suite + Ledger', roi: '3 months' },
  { package: 'Banking Sovereign', price: '$8,000,000+', includes: '+ On-premise, air-gapped, custom', roi: '2 months' },
];
          ...
          <div className="space-y-12">
            <section>
              ...
              <div className="grid grid-cols-3 gap-6">
                {[
                  { title: 'Basel III Endgame Ready', desc: 'Automated RWA computation and capital adequacy monitoring with full audit trail', icon: '📊' },
                  { title: 'Real-Time Fraud Detection', desc: 'AI-powered transaction anomaly detection that identified 3 vulnerability patterns banks missed', icon: '🛡️' },
                  { title: 'CFPB 1071 Compliance', desc: 'Panopticon flagged compliance gaps 6 months before auditors found them', icon: '📋' },
                ].map(item => (
            ...
            <section>
              ...
              <div className="space-y-4">
                {[
                  { org: 'Regional Bank ($8B assets)', quote: 'Reduced fraud losses 40% via CendiaAegis™ RedTeam simulations. The Council identified 3 vulnerability patterns our existing systems missed.', metric: '40% fraud reduction' },
                  { org: 'Credit Union ($2B assets)', quote: 'Cut regulatory response time from 14 days to 3 days. Panopticon flagged CFPB 1071 gaps 6 months before our compliance team.', metric: '14d → 3d response' },
                  { org: 'Asset Manager ($15B AUM)', quote: 'Portfolio rebalancing decisions that took 2 weeks now happen in 2 hours with full audit trail.', metric: '2wk → 2hr decisions' },
                ].map((cs, idx) => (
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-2">
                {compliance.map(c => (
                  <span key={c} className="px-4 py-2 bg-green-500/10 text-green-400 rounded-lg border border-green-500/30 font-medium">{c}</span>
                ))}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="bg-neutral-800 rounded-xl border border-neutral-700 overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody>
                    {[
                      { system: 'Core Banking (FIS, Jack Henry)', difficulty: 'medium', timeline: '4-8 weeks', notes: 'API-based, standard connectors' },
                      { system: 'Trading Platforms (Bloomberg, Refinitiv)', difficulty: 'easy', timeline: '1-2 weeks', notes: 'REST APIs available' },
                      { system: 'Legacy Mainframe (AS/400, COBOL)', difficulty: 'hard', timeline: '3-6 months', notes: 'Requires services engagement' },
                      { system: 'Risk Systems (SAS, Moody\'s)', difficulty: 'medium', timeline: '4-6 weeks', notes: 'Batch file integration typical' },
                    ].map(int => (
          ...
          <div className="space-y-12">
            <section>
              ...
              <div className="grid grid-cols-4 gap-6">
                {pricing.map((pkg, idx) => (
                  <div key={pkg.package} className={`rounded-xl p-6 border ${idx === 2 ? 'bg-primary-900/20 border-primary-500' : 'bg-neutral-800 border-neutral-700'}`}>
