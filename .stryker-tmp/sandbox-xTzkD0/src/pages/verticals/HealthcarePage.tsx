...
const agents: Agent[] = [
  { code: 'cmio', name: 'Chief Medical Info Officer', purpose: 'Health IT strategy, EHR optimization, clinical workflows', model: 'llama3.3:70b' },
  { code: 'pso', name: 'Patient Safety Officer', purpose: 'Clinical safety, adverse event analysis, quality metrics', model: 'qwq:32b' },
  { code: 'hco', name: 'Healthcare Compliance', purpose: 'HIPAA, Stark Law, Anti-Kickback, billing compliance', model: 'qwq:32b' },
  { code: 'cod', name: 'Clinical Operations Director', purpose: 'Patient flow, staffing optimization, capacity planning', model: 'llama3.3:70b' },
];

const overlays = [
  { name: 'Clinical Safety Monitor', function: 'Real-time adverse event detection, near-miss patterns', integrates: 'CendiaAegis™' },
  { name: 'CMS Compliance Tracker', function: 'CoP changes, billing rule updates, quality measure shifts', integrates: 'CendiaPanopticon™' },
  { name: 'Physician Knowledge Vault', function: 'Retiring physician expertise capture, protocol preservation', integrates: 'CendiaEternal™' },
  { name: 'Capacity Predictor', function: 'ED surge forecasting, staffing optimization', integrates: 'CendiaPredict™' },
];

const pillars = [
  { name: 'Helm', application: 'Patient census dashboards, quality metrics, financial performance' },
...
const caseStudies: CaseStudy[] = [
  { org: 'Regional Health System (12 hospitals)', quote: 'Reduced ED boarding time 34% by accelerating discharge decisions. Council identified 7 bottleneck patterns in care coordination.', metric: '34% faster discharge', anonymized: true },
  { org: 'Academic Medical Center', quote: 'Captured 40+ years of retiring surgeon expertise via CendiaEternal™. New residents access institutional knowledge instantly.', metric: '40+ years preserved', anonymized: false },
  { org: 'Rural Hospital Network', quote: 'Avoided $2.3M in CMS penalties. Panopticon flagged CoP violations 90 days before survey.', metric: '$2.3M saved', anonymized: false },
];

const integrations = [
  { system: 'Epic/Cerner EHR', difficulty: 'medium', timeline: '6-12 weeks', notes: 'FHIR R4 APIs, requires IT engagement' },
  { system: 'Meditech', difficulty: 'medium', timeline: '8-12 weeks', notes: 'HL7 v2 typical, FHIR emerging' },
  { system: 'Legacy (CPSI, Allscripts)', difficulty: 'hard', timeline: '3-6 months', notes: 'Often requires middleware' },
  { system: 'Revenue Cycle (Optum, Waystar)', difficulty: 'easy', timeline: '2-4 weeks', notes: 'Standard API connectors' },
];

const compliance = ['HIPAA', 'HITECH', 'Stark Law', 'Anti-Kickback', 'Joint Commission', 'CMS CoPs', 'State Health Laws', '21st Century Cures Act', 'MACRA/MIPS', 'Price Transparency'];

const pricing = [
  { package: 'Healthcare Starter', price: '$100,000', includes: '8 Pillars + 4 Healthcare Agents', roi: '8 months' },
  { package: 'Healthcare Professional', price: '$1,200,000', includes: '+ Aegis, Panopticon, Eternal', roi: '5 months' },
  { package: 'Healthcare Enterprise', price: '$5,000,000', includes: '+ Full Guardian Suite', roi: '3 months' },
  { package: 'Healthcare Sovereign', price: '$12,000,000+', includes: '+ Air-gapped, custom models', roi: '2 months' },
];
          ...
          <div className="space-y-12">
            ...
            <section>
              ...
              <div className="grid grid-cols-3 gap-6">
                {[
                  { title: 'HIPAA-First Architecture', desc: 'Built from the ground up for PHI protection with automatic audit logging of every access', icon: '🔒' },
                  { title: 'CMS AI Transparency Ready', desc: '2025 regulations mandate AI audit trails for clinical decisions—we\'re already compliant', icon: '📋' },
                  { title: 'Air-Gapped Deployment', desc: 'Data never leaves your infrastructure. SCIF-ready for sensitive health systems', icon: '🛡️' },
                ].map(item => (
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-2">
                {compliance.map(c => (
                  <span key={c} className="px-4 py-2 bg-blue-500/10 text-blue-400 rounded-lg border border-blue-500/30 font-medium">
                    {c}
                  </span>
                ))}
              </div>
          ...
          <div className="space-y-12">
            ...
            <section>
              ...
              <div className="grid grid-cols-5 gap-4">
                {pillars.map(pillar => (
                  <div key={pillar.name} className="bg-neutral-800 rounded-xl p-4 border border-neutral-700 text-center">
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-6">
                {[
                  { name: 'CendiaAegis™', use: 'Ransomware defense, medical device security, threat containment' },
                  { name: 'CendiaPanopticon™', use: 'CMS regulations, Joint Commission, state health laws' },
                  { name: 'CendiaEternal™', use: 'Clinical knowledge preservation, physician expertise capture' },
                  { name: 'CendiaVox™', use: 'Patient advocacy, community health input, ethics committee' },
                ].map(service => (
                  <div key={service.name} className="bg-neutral-800 rounded-xl p-6 border border-neutral-700">
                      ...
                      <tr key={int.system} className="border-t border-neutral-700">
                        ...
                        <td className="p-4">
                          <span className={`px-2 py-1 rounded text-xs font-medium ${
                            int.difficulty === 'easy' ? 'bg-green-500/20 text-green-400' :
                            int.difficulty === 'medium' ? 'bg-yellow-500/20 text-yellow-400' :
          ...
          <div className="space-y-12">
            <section>
              ...
              <div className="grid grid-cols-4 gap-6">
                {pricing.map((pkg, idx) => (
                  <div key={pkg.package} className={`rounded-xl p-6 border ${
                    idx === 2 ? 'bg-primary-900/20 border-primary-500' : 'bg-neutral-800 border-neutral-700'
                  }`}>
                    {idx === 2 && (
                      <span className="text-xs bg-primary-500 text-white px-2 py-1 rounded mb-3 inline-block">Most Popular</span>
                    )}
