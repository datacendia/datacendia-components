    ...
    <div className="fixed bottom-4 right-4 z-50 w-[480px] max-h-[600px] bg-slate-900 rounded-xl shadow-2xl border border-slate-700 overflow-hidden flex flex-col">
      ...
      <div className="px-4 py-2 bg-slate-800/50 border-b border-slate-700/50 flex items-center gap-4 text-xs">
        <div className="flex items-center gap-1.5">
          <div className={`w-2 h-2 rounded-full ${stats.isProcessing ? 'bg-green-500 animate-pulse' : 'bg-slate-500'}`} />
          <span className="text-slate-400">{stats.isProcessing ? 'Processing' : 'Idle'}</span>
        </div>
              ...
              errors.map(error => (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <button
                      onClick={() => AutoHealService.requestFix(error.id)}
                      className="px-2 py-1 rounded bg-cyan-500/20 text-cyan-400 text-xs hover:bg-cyan-500/30 transition-colors"
                    >
                      Fix
                    </button>
                  </div>
