...
class PersonaForgeService {
  ...
  private initializeDefaultPersonas(): void {
    ...
    const defaults: Partial<DigitalPersona>[] = [
      ...
      },
      {
        ...
        specializations: ['Enterprise Architecture', 'Cloud Infrastructure', 'Cybersecurity', 'Digital Transformation'],
      },
      {
        role: 'clo',
        name: 'Digital CLO',
        ...
        specializations: ['Corporate Law', 'Contract Law', 'Regulatory Compliance', 'IP Protection'],
      },
      {
        role: 'chro',
        name: 'Digital CHRO',
  ...
  private getDefaultCapabilities(role: PersonaRole): PersonaCapability[] {
    const capabilities: Record<PersonaRole, PersonaCapability[]> = {
      cfo: [
        { id: 'budget-analysis', name: 'Budget Analysis', description: 'Analyze and forecast budgets', accuracy: 94, usageCount: 0, examples: ['Q4 budget projection', 'Cost optimization'] },
        { id: 'financial-reporting', name: 'Financial Reporting', description: 'Generate financial reports', accuracy: 97, usageCount: 0, examples: ['Monthly P&L', 'Cash flow statement'] },
        { id: 'risk-assessment', name: 'Financial Risk Assessment', description: 'Evaluate financial risks', accuracy: 91, usageCount: 0, examples: ['Currency exposure', 'Credit risk'] },
      ],
      cio: [
        { id: 'tech-strategy', name: 'Technology Strategy', description: 'Develop IT roadmaps', accuracy: 92, usageCount: 0, examples: ['Cloud migration', 'Tech stack decisions'] },
        { id: 'vendor-evaluation', name: 'Vendor Evaluation', description: 'Assess technology vendors', accuracy: 88, usageCount: 0, examples: ['SaaS selection', 'Contract negotiation'] },
      ],
      clo: [
        { id: 'contract-review', name: 'Contract Review', description: 'Analyze legal contracts', accuracy: 96, usageCount: 0, examples: ['NDA review', 'Vendor agreements'] },
        { id: 'compliance-check', name: 'Compliance Check', description: 'Verify regulatory compliance', accuracy: 94, usageCount: 0, examples: ['GDPR audit', 'SOX compliance'] },
      ],
      chro: [
        { id: 'talent-analysis', name: 'Talent Analysis', description: 'Analyze workforce data', accuracy: 85, usageCount: 0, examples: ['Retention risk', 'Skills gap'] },
      ],
      ciso: [
        { id: 'threat-assessment', name: 'Threat Assessment', description: 'Evaluate security threats', accuracy: 93, usageCount: 0, examples: ['APT analysis', 'Vulnerability scoring'] },
      ],
      cso: [
        { id: 'strategic-analysis', name: 'Strategic Analysis', description: 'Market and competitive analysis', accuracy: 90, usageCount: 0, examples: ['Market entry', 'Competitive positioning'] },
      ],
      cro: [
        { id: 'revenue-forecast', name: 'Revenue Forecasting', description: 'Predict revenue trends', accuracy: 88, usageCount: 0, examples: ['Pipeline analysis', 'Quota setting'] },
      ],
      coo: [
        { id: 'ops-optimization', name: 'Operations Optimization', description: 'Improve operational efficiency', accuracy: 91, usageCount: 0, examples: ['Process improvement', 'Resource allocation'] },
      ],
      cpo: [
        { id: 'product-strategy', name: 'Product Strategy', description: 'Product roadmap planning', accuracy: 89, usageCount: 0, examples: ['Feature prioritization', 'Market fit'] },
      ],
      custom: [],
    };
  ...
  private generateFallbackResponse(persona: DigitalPersona, message: string): string {
    ...
    return roleResponses.default || 'I\'m here to help. Could you provide more details about your question?';
  }
  ...
  getStats(): {
    ...
    return {
      ...
      trainingPersonas: personas.filter(p => p.status !== 'ready' && p.status !== 'not_started').length,
