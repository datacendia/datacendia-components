...
export function traitsConflict(traitId1: string, traitId2: string): boolean {
  ...
  return Boolean(
    ...
    trait2.conflictsWith?.includes(traitId1)
  );
}
 ...
 */
export function validateTraitCombination(traitIds: string[]): { valid: boolean; conflicts: string[][] } {
...
export function getTraitCategories(): { id: TraitCategory; name: string; description: string }[] {
  return [
    { id: 'communication_style', name: 'Communication Style', description: 'How the agent expresses ideas' },
    ...
    { id: 'conflict_approach', name: 'Conflict Approach', description: 'How disagreements are handled' },
    ...
    { id: 'emotional_expression', name: 'Emotional Expression', description: 'How emotions are shown' },
