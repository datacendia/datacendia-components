  ...
  return (
    <div className="relative h-64 flex items-center justify-center">
      <svg className="w-full h-full" viewBox="0 0 600 200" preserveAspectRatio="xMidYMid meet">
        ...
        <text x={endX} y="195" fill="#6B7280" fontSize="10" textAnchor="middle">
          +{horizon}d
        </text>
...
const OutcomeCard: React.FC<{ outcome: ScenarioOutcome }> = ({ outcome }) => {
  ...
  return (
    <div className={cn('bg-neutral-800/50 rounded-lg p-4 border-t-2', config.border)}>
      ...
      <p
        className={cn(
          ...
          outcome.percentage > 0
            ? 'text-green-400'
            : outcome.percentage < -5
              ? 'text-red-400'
              : 'text-neutral-300'
        )}
      >
        {isPositive ? '+' : ''}
        {outcome.percentage}% Rev
      </p>
    </div>
  );
};
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <span className="text-xs text-neutral-400 uppercase tracking-wider">SIMULATION:</span>
            <select
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            onClick={() => {
              const summary =
                `Analyze the ${selectedSimulation.name} scenario with ${timeHorizon}-day horizon. ` +
