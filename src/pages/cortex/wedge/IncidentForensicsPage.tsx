...
const ForensicReportView: React.FC<{ report: ForensicReport; onNew: () => void }> = ({ report, onNew }) => {
  const [tab, setTab] = useState<'timeline' | 'rootcause' | 'impact' | 'evidence' | 'legal' | 'prevention' | 'cases'>('timeline');
  ...
  const tabs = [
    ...
    { id: 'cases' as const, label: 'Case Management', icon: <Building2 className="w-4 h-4" /> },
  ];
...
// =============================================================================
// ENTERPRISE PLATINUM: CASE MANAGEMENT & SIEM INTEGRATION
// =============================================================================

const CaseManagementPanel: React.FC = () => {
  return (
    <div className="space-y-6">
      {/* Active Cases */}
      <div className="bg-sovereign-card border border-sovereign-border rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs px-2 py-0.5 bg-red-500/10 text-red-400 rounded-full">4 open</span>
        </div>
        <div className="space-y-2">
          {[
            { id: 'INC-M8K7', title: 'Chatbot quoted incorrect refund', severity: 'critical', assignee: 'Legal Team', status: 'investigating', age: '2 days' },
            { id: 'INC-P3Q2', title: 'AI screening rejected qualified candidate', severity: 'high', assignee: 'HR + Legal', status: 'evidence_collection', age: '5 days' },
            { id: 'INC-R9T4', title: 'Model hallucinated compliance citation', severity: 'medium', assignee: 'Compliance', status: 'root_cause', age: '1 week' },
            { id: 'INC-V2X1', title: 'PII exposed in AI summary', severity: 'high', assignee: 'Security', status: 'remediation', age: '2 weeks' },
          ].map((c, i) => (
            <div key={i} className="flex items-center justify-between py-3 border-b border-sovereign-border last:border-0">
              <div className="flex items-center gap-3">
                <span className={cn('w-2 h-2 rounded-full', c.severity === 'critical' ? 'bg-red-500' : c.severity === 'high' ? 'bg-orange-500' : 'bg-yellow-500')} />
                <div>
                  <div className="text-sm text-gray-200"><span className="font-mono text-gray-500 mr-2">{c.id}</span>{c.title}</div>
                  <div className="text-xs text-gray-600">Assigned: {c.assignee} · Status: {c.status.replace('_', ' ')} · {c.age}</div>
                </div>
              </div>
              <span className={cn('text-xs px-2 py-0.5 rounded-full border', c.severity === 'critical' ? 'bg-red-500/10 text-red-400 border-red-500/20' : c.severity === 'high' ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20')}>{c.severity}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* SIEM Integration */}
      <div className="bg-sovereign-card border border-sovereign-border rounded-xl p-5">
        <h3 className="text-sm font-semibold text-white mb-3">SIEM & Ticketing Integration</h3>
        <div className="grid grid-cols-2 gap-3">
          {[
            { name: 'Splunk', status: 'connected', color: 'green' },
            { name: 'ServiceNow', status: 'connected', color: 'green' },
            { name: 'Jira', status: 'connected', color: 'green' },
            { name: 'PagerDuty', status: 'configured', color: 'yellow' },
          ].map((integration, i) => (
            <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-sovereign-dark border border-sovereign-border">
              <span className="text-sm text-gray-300">{integration.name}</span>
              <div className="flex items-center gap-1.5">
                <div className={cn('w-2 h-2 rounded-full', integration.color === 'green' ? 'bg-green-500' : 'bg-yellow-500')} />
      ...
      {/* Forensic Templates */}
      <div className="bg-sovereign-card border border-sovereign-border rounded-xl p-5">
        <h3 className="text-sm font-semibold text-white mb-3">Forensic Report Templates</h3>
        <div className="space-y-2">
          {[
            { name: 'Customer-Facing AI Error', desc: 'Pre-configured for chatbot/support AI incidents', uses: 12 },
            { name: 'HR/Hiring Bias Investigation', desc: 'EEOC-compliant investigation template', uses: 3 },
            { name: 'Financial Decision Error', desc: 'SOX-compliant AI decision forensics', uses: 7 },
            { name: 'Data Breach via AI', desc: 'GDPR Article 33 notification package', uses: 2 },
          ].map((tmpl, i) => (
            <div key={i} className="flex items-center justify-between py-2 border-b border-sovereign-border last:border-0">
              <div>
                <div className="text-sm text-gray-200">{tmpl.name}</div>
                <div className="text-xs text-gray-500">{tmpl.desc}</div>
              </div>
              <span className="text-xs text-gray-600">{tmpl.uses} uses</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Total Incidents', value: '23', color: 'text-white' },
          { label: 'Avg Resolution', value: '4.2 days', color: 'text-blue-400' },
          { label: 'Evidence Items', value: '147', color: 'text-purple-400' },
          { label: 'Prevented Repeat', value: '89%', color: 'text-green-400' },
        ].map((m, i) => (
          <div key={i} className="bg-sovereign-card border border-sovereign-border rounded-xl p-4 text-center">
