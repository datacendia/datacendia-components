/**
 * Page — Ops Agents (Enterprise Edition)
 *
 * Practical, everyday AI agents that execute tasks:
 *   - Report Automation Agent
 *   - Analytics Agent
 *   - NLP Agent (Enterprise)
 *   - Pipeline Builder Agent (Enterprise)
 *
 * All 4 agents unlocked, full quick-prompt library, higher limits,
 * multi-format export, integrity hashing on results.
 *
...
interface OpsTask {
  ...
  prompt: string;
  status: TaskStatus;
  result?: { content: string; format: string; metadata?: Record<string, unknown>; sections?: Array<{ title: string; content: string }> };
  createdAt: string;
  completedAt?: string;
  durationMs?: number;
  error?: string;
...
// Enterprise: full quick-prompt library (3 per agent)
...
function simpleMarkdownToHtml(md: string): string {
  return md
    ...
    .replace(/^## (.+)$/gm, '<h2 class="text-lg font-bold text-zinc-100 mt-5 mb-2">$1</h2>')
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-gradient-to-r from-blue-500/20 to-purple-500/20 border border-blue-500/30 text-blue-300 font-semibold uppercase tracking-wider">Enterprise</span>
            </div>
            <p className="text-xs text-zinc-500">Everyday AI agents that execute tasks — all 4 agents unlocked</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {aiAvailable !== null && (
