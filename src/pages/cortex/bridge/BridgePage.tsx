...
const activeWorkflows: ActiveWorkflow[] = [
  {
    ...
    nodes: [
      ...
      { id: 'n2', type: 'ai', label: 'AI Recommendation: Reallocate Marketing Spend', status: 'completed' },
...
const SLACountdown: React.FC<{ deadline: Date }> = ({ deadline }) => {
  ...
  return (
    <div className={cn(
      ...
      isUrgent ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-neutral-700 text-neutral-300'
    )}>
      SLA: {timeLeft}
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center">
      <div className={cn(
        ...
        {node.sublabel && (
          <p className="text-xs text-neutral-500 mt-1">{node.sublabel}</p>
        )}
      </div>
      {!isLast && (
        <div className="w-px h-8 bg-neutral-700 my-2" />
      )}
    </div>
  );
};
...
export const BridgePage: React.FC = () => {
  ...
  const [activityEvents, setActivityEvents] = useState<ActivityEvent[]>([
    ...
    { id: '4', type: 'error', message: 'Workday sync failed', workflow: 'HR_ONBOARD_02', timestamp: new Date() },
  ]);
  ...
  const [executionHistory, setExecutionHistory] = useState<ExecutionHistory[]>([
    ...
    { id: '4', workflowCode: 'ALERT_ESCAL_01', status: 'success', duration: '8s', timestamp: new Date() },
  ]);
