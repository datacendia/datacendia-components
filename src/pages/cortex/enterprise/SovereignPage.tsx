...
const calculateClusterMetrics = (nodes: GPUNode[], models: DeployedModel[]): ClusterMetrics => {
  ...
  return {
    ...
    avgLatency: onlineNodes.length > 0 
      ...
      : 0,
    tokensPerSecond: Math.round(activeModels.reduce((sum, m) => sum + m.tokensGenerated, 0) / 86400),
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-950 text-white">
      ...
      <div className="bg-gradient-to-r from-indigo-900/30 to-purple-900/30 border-b border-indigo-800/30">
        <div className="max-w-7xl mx-auto px-6 py-3">
          <div className="grid grid-cols-8 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-white">{metrics?.onlineNodes ?? 0}/{metrics?.totalNodes ?? 0}</div>
            ...
            <div>
              <div className="text-2xl font-bold text-white">{metrics?.totalVRAM ? Math.round((metrics.usedVRAM / metrics.totalVRAM) * 100) : 0}%</div>
            ...
            <div>
              <div className="text-2xl font-bold text-cyan-400">{metrics?.requestsPerSecond?.toFixed(1) ?? '0.0'}</div>
              <div className="text-xs text-indigo-300">Req/sec</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-amber-400">{Math.round(metrics?.avgLatency ?? 0)}ms</div>
              <div className="text-xs text-indigo-300">Avg Latency</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-purple-400">{((metrics?.tokensPerSecond ?? 0) / 1000).toFixed(1)}K</div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
                ...
                {nodes.map(node => (
                  <div
                    ...
                    className={`p-4 rounded-xl border cursor-pointer transition-all ${
                      node.status === 'online' ? 'bg-green-900/20 border-green-700/50 hover:border-green-500' :
                      node.status === 'busy' ? 'bg-amber-900/20 border-amber-700/50 hover:border-amber-500' :
                      node.status === 'draining' ? 'bg-blue-900/20 border-blue-700/50 hover:border-blue-500' :
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded ${
                        node.zone === 'air-gapped' ? 'bg-red-600' :
                        node.zone === 'edge' ? 'bg-amber-600' :
                        'bg-indigo-600'
                      }`}>
                        {node.zone}
                      </span>
                      ...
                      <div className="h-1.5 bg-black/30 rounded-full overflow-hidden">
                        <div
                          className={`h-full transition-all ${
                            node.usedVRAM / node.totalVRAM > 0.9 ? 'bg-red-500' :
                            node.usedVRAM / node.totalVRAM > 0.7 ? 'bg-amber-500' :
                            'bg-green-500'
                          }`}
            ...
            <div className="grid grid-cols-3 gap-6">
              ...
              <div className="bg-black/30 rounded-2xl p-6 border border-indigo-800/50">
                ...
                <div className="space-y-3">
                  {models.filter(m => m.status === 'active').slice(0, 5).map(model => (
        ...
        {activeTab === 'nodes' && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
              {nodes.map(node => (
                <div key={node.id} className="bg-black/30 rounded-2xl p-6 border border-indigo-800/50">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-3 py-1 rounded-lg text-sm ${
                        ...
                        node.status === 'busy' ? 'bg-amber-600' :
                        node.status === 'draining' ? 'bg-blue-600' :
                        'bg-red-600'
                      }`}>
                        {node.status}
                      </span>
                      <span className={`px-3 py-1 rounded-lg text-sm ${
                        node.zone === 'air-gapped' ? 'bg-red-900 text-red-300' :
                        node.zone === 'edge' ? 'bg-amber-900 text-amber-300' :
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {node.loadedModels.map(m => (
                          <span key={m} className="text-xs px-2 py-1 bg-indigo-900/50 rounded">{m}</span>
                        ))}
                      </div>
                    </div>
                  </div>
        ...
        {activeTab === 'models' && (
          <div className="space-y-4">
            {models.map(model => (
              <div key={model.id} className="bg-black/30 rounded-2xl p-6 border border-indigo-800/50">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${
                      model.family === 'llama' ? 'bg-blue-900' :
                      model.family === 'mistral' ? 'bg-orange-900' :
                      model.family === 'qwen' ? 'bg-purple-900' :
                      model.family === 'deepseek' ? 'bg-cyan-900' :
                      'bg-neutral-800'
                    }`}>
                      🤖
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`px-3 py-1 rounded-lg text-sm ${
                      ...
                      model.status === 'loading' ? 'bg-amber-600' :
                      model.status === 'sandboxed' ? 'bg-purple-600' :
                      'bg-neutral-600'
                    }`}>
                      {model.status}
                    </span>
                    <span className={`px-3 py-1 rounded-lg text-sm ${
                      model.complianceStatus === 'approved' ? 'bg-green-900 text-green-300' :
                      model.complianceStatus === 'pending' ? 'bg-amber-900 text-amber-300' :
                      model.complianceStatus === 'review' ? 'bg-blue-900 text-blue-300' :
            ...
            <div className="bg-black/30 rounded-2xl p-6 border border-indigo-800/50">
              ...
              <div className="space-y-4">
                {failovers.map(fo => {
                  const primaryModel = models.find(m => m.id === fo.primaryModel);
                  return (
                    <div key={fo.id} className="p-4 bg-black/20 rounded-xl border border-indigo-800/30">
            ...
            {sandboxes.map(sb => (
              <div key={sb.id} className="bg-black/30 rounded-2xl p-6 border border-indigo-800/50">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`px-3 py-1 rounded-lg text-sm ${
                    sb.status === 'approved' ? 'bg-green-600' :
                    sb.status === 'testing' ? 'bg-blue-600' :
                    sb.status === 'review' ? 'bg-amber-600' :
                    sb.status === 'rejected' ? 'bg-red-600' :
                    'bg-neutral-600'
                  }`}>
                    {sb.status}
                  </span>
                  ...
                  {sb.tests.map(test => (
                    <div
                      ...
                      className={`p-3 rounded-xl text-center ${
                        test.status === 'passed' ? 'bg-green-900/30 border border-green-700/50' :
                        test.status === 'running' ? 'bg-blue-900/30 border border-blue-700/50' :
                        test.status === 'failed' ? 'bg-red-900/30 border border-red-700/50' :
