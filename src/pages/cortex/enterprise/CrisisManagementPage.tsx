...
const mockIncidents: Incident[] = [
  {
    ...
    containmentActions: ['Blocked IP range 185.x.x.x', 'Forced password reset for targeted accounts', 'Enabled enhanced monitoring'],
    timeline: [
      { id: '1', timestamp: new Date(Date.now() - 2 * 60 * 60 * 1000), actor: 'SIEM', action: 'Alert Generated', details: '50+ failed login attempts detected', phase: 'detection' },
      { id: '2', timestamp: new Date(Date.now() - 1.9 * 60 * 60 * 1000), actor: 'SOC Analyst', action: 'Triage Started', details: 'Confirmed malicious pattern, escalated to P2', phase: 'triage' },
      { id: '3', timestamp: new Date(Date.now() - 1.5 * 60 * 60 * 1000), actor: 'Security Team', action: 'IP Block Deployed', details: 'Firewall rules updated', phase: 'containment' },
    ],
  },
  {
    id: 'INC-2025-002',
    ...
    containmentActions: ['Prepared holding statement', 'Identified spokesperson', 'Drafted detailed response'],
    timeline: [
      { id: '1', timestamp: new Date(Date.now() - 24 * 60 * 60 * 1000), actor: 'Social Listening', action: 'Alert', details: 'Article gaining traction, 500+ shares', phase: 'detection' },
      { id: '2', timestamp: new Date(Date.now() - 23 * 60 * 60 * 1000), actor: 'CMO', action: 'War Room Activated', details: 'Crisis team assembled', phase: 'triage' },
      { id: '3', timestamp: new Date(Date.now() - 22 * 60 * 60 * 1000), actor: 'Communications', action: 'Holding Statement', details: 'Released initial response', phase: 'containment' },
      { id: '4', timestamp: new Date(Date.now() - 18 * 60 * 60 * 1000), actor: 'CEO', action: 'Full Response Published', details: 'Detailed blog post addressing concerns', phase: 'recovery' },
    ],
  ...
  },
  {
    ...
    containmentActions: ['Scaled read replicas', 'Implemented query caching', 'Identified slow queries'],
    timeline: [],
...
const mockWarRoom: WarRoom = {
  ...
  decisions: [
    { text: 'Block suspicious IP range immediately', madeBy: 'Sarah Chen', timestamp: new Date(Date.now() - 1.5 * 60 * 60 * 1000) },
    { text: 'Force password reset for all executive accounts', madeBy: 'Sarah Chen', timestamp: new Date(Date.now() - 1.3 * 60 * 60 * 1000) },
  ],
};
...
const getPhaseColor = (phase: IncidentPhase) => {
  switch (phase) {
    case 'detection': return 'bg-purple-500/20 text-purple-400';
    case 'triage': return 'bg-orange-500/20 text-orange-400';
    case 'containment': return 'bg-yellow-500/20 text-yellow-400';
    case 'eradication': return 'bg-blue-500/20 text-blue-400';
    case 'recovery': return 'bg-cyan-500/20 text-cyan-400';
    case 'post-mortem': return 'bg-indigo-500/20 text-indigo-400';
...
export const CrisisManagementPage: React.FC = () => {
  ...
  const phases: IncidentPhase[] = ['detection', 'triage', 'containment', 'eradication', 'recovery', 'post-mortem', 'closed'];
    ...
    <div className="min-h-screen bg-neutral-900 text-white p-6">
      ...
      {activeTab === 'dashboard' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              { label: 'Active Incidents', value: activeIncidents.length, color: 'text-red-400', bg: 'bg-red-500/10' },
              { label: 'P1 Critical', value: mockIncidents.filter(i => i.severity === 'P1').length, color: 'text-red-500', bg: 'bg-red-500/10' },
              { label: 'Mean Time to Contain', value: '2.4h', color: 'text-yellow-400', bg: 'bg-yellow-500/10' },
              { label: 'Resolved This Month', value: '12', color: 'text-green-400', bg: 'bg-green-500/10' },
            ].map(stat => (
              <div key={stat.label} className={`${stat.bg} rounded-xl p-6 border border-neutral-700`}>
            ...
            <div className="bg-neutral-800 rounded-xl border border-neutral-700 p-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  { label: 'Notify Stakeholders', icon: '📧', color: 'bg-blue-600 hover:bg-blue-700' },
                  { label: 'Start Council Deliberation', icon: '🧠', color: 'bg-purple-600 hover:bg-purple-700' },
                  { label: 'Log Decision', icon: '📝', color: 'bg-green-600 hover:bg-green-700' },
                  { label: 'Request Resources', icon: '🔧', color: 'bg-orange-600 hover:bg-orange-700' },
                  { label: 'Close Incident', icon: '✅', color: 'bg-neutral-600 hover:bg-neutral-700' },
                ].map(action => (
                  <button
                    key={action.label}
            ...
            <div className="bg-neutral-800 rounded-xl border border-neutral-700 p-6">
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={channel.name} className="flex items-center justify-between p-2 bg-neutral-900 rounded">
                    <span className="text-sm">{channel.name}</span>
                    <span className={`text-xs px-2 py-0.5 rounded ${
                      channel.status === 'connected' ? 'bg-green-500/20 text-green-400' :
                      channel.status === 'ready' ? 'bg-blue-500/20 text-blue-400' :
      ...
      {activeTab === 'playbooks' && (
        <div className="grid grid-cols-3 gap-4">
          {[
            { name: 'Security Incident Response', type: 'security', steps: 8, lastUsed: '2 days ago', sla: '15 min triage' },
            { name: 'PR Crisis Response', type: 'pr', steps: 6, lastUsed: '1 week ago', sla: '60 min statement' },
            { name: 'Data Breach Protocol', type: 'security', steps: 12, lastUsed: 'Never', sla: '4 hr notification' },
            { name: 'Service Outage', type: 'operational', steps: 7, lastUsed: '3 days ago', sla: '5 min detection' },
            { name: 'Compliance Violation', type: 'compliance', steps: 10, lastUsed: '2 weeks ago', sla: '24 hr remediation' },
            { name: 'Legal Threat', type: 'legal', steps: 5, lastUsed: 'Never', sla: '4 hr legal review' },
          ].map(playbook => (
            <div key={playbook.name} className="bg-neutral-800 rounded-xl border border-neutral-700 p-6 hover:border-primary-500 cursor-pointer transition-all">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedIncident.affectedSystems.map(sys => (
                      <span key={sys} className="px-3 py-1 bg-neutral-700 rounded-full text-sm">{sys}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                <div className="space-y-3">
                  {selectedIncident.timeline.map(event => (
                    <div key={event.id} className="flex items-start gap-3 p-3 bg-neutral-900 rounded-lg">
