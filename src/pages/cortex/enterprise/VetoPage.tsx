                  ...
                  <div key={agent.id} className="p-3 bg-black/20 rounded-xl">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      {agent.canBlockAutomatic && (
                        <span className="px-2 py-0.5 bg-red-900/50 rounded text-red-300">Can Block</span>
                      )}
        ...
        {activeTab === 'proposals' && (
          <div className="space-y-4">
            {decisions.map(decision => (
              <div key={decision.id} className="bg-black/30 rounded-2xl p-6 border border-red-800/50">
                  ...
                  {decision.reviews.map(review => {
                    ...
                    return (
                      <div key={review.id} className={`p-3 rounded-xl ${
                        review.status === 'vetoed' ? 'bg-red-900/30 border border-red-600/50' :
                        review.status === 'approved' ? 'bg-green-900/30 border border-green-600/50' :
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap gap-1">
                    {agent.jurisdiction.map(j => (
                      <span key={j} className="px-2 py-0.5 bg-red-900/30 rounded text-xs text-red-300">
                        {j.replace(/_/g, ' ')}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    {agent.requiresHumanOverride && (
                      <span className="px-2 py-1 bg-purple-600 rounded text-xs">Human Override Required</span>
                    )}
                  </div>
                </div>
              </div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-0.5 rounded text-xs ${
                          review.status === 'vetoed' ? 'bg-red-600' :
                          review.status === 'approved' ? 'bg-green-600' : 'bg-amber-600'
                        }`}>
                          {review.status}
                        </span>
                        ...
                        {review.concerns.map(c => (
                          <div key={c.id} className="text-xs p-2 bg-black/20 rounded mb-1">
                            <span className={`font-semibold ${
                              ...
                              c.severity === 'medium' ? 'text-amber-400' : 'text-white/60'
                            }`}>{c.severity.toUpperCase()}</span>: {c.description}
                          </div>
                        ))}
                      </div>
                    )}
