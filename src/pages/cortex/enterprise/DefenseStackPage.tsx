...
const CLASSIFICATION_CONFIG: Record<ClassificationLevel, { color: string; label: string; bg: string }> = {
...
const generateThreatScenarios = (): ThreatScenario[] => [
  {
    ...
    indicators: ['Spear phishing campaigns', 'C2 beacon activity', 'Lateral movement patterns', 'Data staging'],
    countermeasures: ['Enhanced email filtering', 'Network segmentation', 'EDR deployment', 'Zero trust enforcement'],
  ...
  },
  {
    ...
    indicators: ['Vendor anomalies', 'Code integrity failures', 'Unexpected dependencies', 'Certificate issues'],
    countermeasures: ['SBOM verification', 'Vendor security assessments', 'Code signing enforcement', 'Continuous monitoring'],
  ...
  },
  {
    ...
    impact: 'significant',
    timeframe: 'Ongoing',
    indicators: ['Coordinated social media activity', 'Bot network activation', 'Narrative amplification', 'Deep fake content'],
    countermeasures: ['Media monitoring', 'Attribution analysis', 'Counter-narrative development', 'Public awareness'],
  ...
  },
  {
    ...
    countermeasures: ['Physical security hardening', 'Personnel vetting', 'Redundancy planning', 'Rapid response teams'],
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              {securityPosture.certifications.slice(0, 3).map(cert => (
                <span key={cert} className="px-2 py-1 bg-green-900/30 border border-green-600/30 rounded text-xs text-green-400">
                  ✓ {cert}
                </span>
              ))}
      ...
      <div className="bg-gradient-to-r from-slate-900/50 to-neutral-900/50 border-b border-slate-800/50">
        <div className="max-w-7xl mx-auto px-6 py-3">
          <div className="grid grid-cols-8 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-purple-400">{warGames.filter(w => w.status === 'active').length}</div>
              <div className="text-xs text-slate-400">Active Wargames</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-blue-400">{deploymentNodes.filter(n => n.status === 'online').length}</div>
              ...
              <div className="bg-red-900/20 rounded-2xl p-6 border border-red-700/50">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {activeThreats.map(threat => (
                    <div key={threat.id} className="p-4 bg-red-900/30 rounded-xl border border-red-700/30">
                      ...
                      <div className={`inline-block px-2 py-0.5 rounded text-xs ${
                        threat.impact === 'catastrophic' ? 'bg-red-600' :
                        threat.impact === 'critical' ? 'bg-orange-600' :
                        'bg-amber-600'
                      }`}>{threat.impact.toUpperCase()} IMPACT</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-black/30 rounded-2xl p-6 border border-slate-700/50">
                ...
                <div className="space-y-2">
                  {securityPosture.certifications.map(cert => (
                    <div key={cert} className="flex items-center justify-between p-3 bg-green-900/20 rounded-lg border border-green-700/30">
            ...
            <div className="bg-black/30 rounded-2xl p-6 border border-slate-700/50">
              ...
              <div className="grid grid-cols-4 gap-4">
                {deploymentNodes.map(node => {
                  ...
                  return (
                    <div key={node.id} className="p-4 bg-black/20 rounded-xl border border-slate-700/30">
                      <div className="flex items-center justify-between mb-2">
        ...
        {activeTab === 'threats' && (
          <div className="space-y-4">
            {threatScenarios.map(threat => (
              <div key={threat.id} className={`bg-black/30 rounded-2xl p-6 border ${
                threat.probability > 0.7 ? 'border-red-700/50' :
                threat.probability > 0.4 ? 'border-amber-700/50' :
                'border-slate-700/50'
              }`}>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    <div className={`text-3xl font-bold ${
                      threat.probability > 0.7 ? 'text-red-400' :
                      threat.probability > 0.4 ? 'text-amber-400' :
                      'text-green-400'
                    }`}>{(threat.probability * 100).toFixed(0)}%</div>
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-700/30">
                  <span className={`px-3 py-1 rounded text-sm ${
                    threat.impact === 'catastrophic' ? 'bg-red-600' :
                    threat.impact === 'critical' ? 'bg-orange-600' :
                    threat.impact === 'significant' ? 'bg-amber-600' :
                    'bg-blue-600'
                  }`}>{threat.impact.toUpperCase()} IMPACT</span>
                ...
                <div key={wg.id} className="bg-black/30 rounded-2xl p-6 border border-slate-700/50">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`px-3 py-1 rounded-lg text-sm ${
                      wg.status === 'active' ? 'bg-green-600' :
                      wg.status === 'complete' ? 'bg-blue-600' :
                      'bg-neutral-600'
                    }`}>{wg.status.toUpperCase()}</span>
                  </div>
        ...
        {activeTab === 'documents' && (
          <div className="space-y-4">
            ...
            {classifiedDocs.map(doc => {
              ...
              return (
                <div key={doc.id} className="bg-black/30 rounded-2xl p-6 border border-slate-700/50">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                    ...
                    <span className={`px-3 py-1 rounded-lg text-sm ${
                      ...
                      doc.status === 'processing' ? 'bg-amber-600' :
                      'bg-blue-600'
                    }`}>{doc.status}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {doc.compartments.map(comp => (
                      <span key={comp} className="px-2 py-1 bg-red-900/30 border border-red-700/30 rounded text-xs">
                        {comp}
                      </span>
                    ))}
                  </div>
        ...
        {activeTab === 'deployment' && (
          <div className="grid grid-cols-2 gap-6">
            {deploymentNodes.map(node => {
              const classConfig = CLASSIFICATION_CONFIG[node.classification];
              return (
                <div key={node.id} className="bg-black/30 rounded-2xl p-6 border border-slate-700/50">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-black/30 rounded-2xl border border-slate-700/50 overflow-hidden">
              ...
              {auditEvents.map(event => {
                ...
                return (
                  <div key={event.id} className="grid grid-cols-7 gap-4 p-4 border-b border-slate-800/30 text-sm">
