 ...
 *
 * @exports CarbonAwareSchedulerPage
 * @module pages/cortex/enterprise/CarbonAwareSchedulerPage
 */
        ...
        <div className="bg-gradient-to-r from-green-900/20 to-emerald-900/20 border border-green-800 rounded-lg p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${
              budget.status === 'on_track' ? 'bg-green-900/30 text-green-300' :
              budget.status === 'warning' ? 'bg-yellow-900/30 text-yellow-300' :
          ...
          <div className="grid grid-cols-4 gap-4">
            <div>
              ...
              <div className="text-2xl font-bold text-white">{budget.budgetKgCO2} kg</div>
            </div>
            <div>
              <div className="text-sm text-gray-400 mb-1">Used</div>
              <div className="text-2xl font-bold text-white">{budget.usedKgCO2.toFixed(1)} kg</div>
            </div>
            <div>
              <div className="text-sm text-gray-400 mb-1">Remaining</div>
              <div className="text-2xl font-bold text-green-400">{budget.remainingKgCO2.toFixed(1)} kg</div>
            </div>
            <div>
              <div className="text-sm text-gray-400 mb-1">Forecast</div>
              <div className="text-2xl font-bold text-white">{budget.forecastKgCO2.toFixed(1)} kg</div>
            </div>
          </div>
          <div className="mt-4">
        ...
        <div className="bg-gray-800 rounded-lg border border-gray-700 p-6">
          ...
          <div className="space-y-3">
            {carbonIntensity.slice(0, 6).map((region) => (
              <div key={region.region} className="flex items-center justify-between p-3 bg-gray-700/50 rounded-lg">
                <div className="flex items-center gap-3">
                  <MapPin className="w-4 h-4 text-gray-400" />
                  <div>
                    <div className="font-medium text-sm text-white">{region.region}</div>
                    <div className="text-xs text-gray-400">{region.source}</div>
                  </div>
                </div>
        ...
        {workloads.length === 0 ? (
          ...
          <div className="space-y-3">
            {workloads.map((workload) => (
              <div
                key={workload.id}
                className="p-4 border border-gray-700 rounded-lg hover:border-green-600 transition-colors"
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <div className="font-medium text-white">{workload.name}</div>
                    <span className="px-2 py-0.5 bg-gray-700 text-gray-300 text-xs rounded">
                      {workload.type}
                    </span>
                    <span className={`px-2 py-0.5 text-xs rounded ${
                      workload.status === 'completed' ? 'bg-green-900/30 text-green-300' :
                      workload.status === 'running' ? 'bg-blue-900/30 text-blue-300' :
                      workload.status === 'deferred' ? 'bg-yellow-900/30 text-yellow-300' :
