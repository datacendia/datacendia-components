...
const STARTER_TEMPLATES = [
  { id: 'vendors', name: 'Common Vendors', icon: '🏢', count: 15, description: 'Cloud, SaaS, infrastructure' },
  { id: 'partners', name: 'Partnership Types', icon: '🤝', count: 8, description: 'Channel, tech, strategic' },
  { id: 'competitors', name: 'Competitor Framework', icon: '🎯', count: 5, description: 'Direct, indirect, emerging' },
  { id: 'investors', name: 'Investor Categories', icon: '💰', count: 6, description: 'VC, PE, strategic, angel' },
];

// Opportunity detection signals explanation
const OPPORTUNITY_SIGNALS = [
  { source: 'Contracts', icon: '📝', description: 'Renewal dates, spend changes, term modifications' },
  { source: 'CRM', icon: '📊', description: 'Deal pipeline, relationship health, engagement' },
  { source: 'News & PR', icon: '📰', description: 'M&A activity, leadership changes, funding rounds' },
  { source: 'Market Data', icon: '📈', description: 'Competitor moves, industry trends, market shifts' },
  { source: 'Internal Signals', icon: '📡', description: 'Usage patterns, support tickets, NPS feedback' },
];
...
export const SymbiontPage: React.FC = () => {
  ...
  const scoredOpportunities = opportunities.map(o => ({
    ...o,
    priority: o.estimatedValue && o.estimatedValue > 500000 ? 'HIGH' : 
              o.estimatedValue && o.estimatedValue > 100000 ? 'MEDIUM' : 'LOW'
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white p-6">
                  ...
                  return (
                    <div 
                      ...
                      style={{ 
                        ...
                        backgroundColor: e.entityType === 'PARTNER' ? 'rgba(16, 185, 129, 0.2)' :
                                        e.entityType === 'VENDOR' ? 'rgba(59, 130, 246, 0.2)' :
                                        e.entityType === 'COMPETITOR' ? 'rgba(239, 68, 68, 0.2)' :
                                        'rgba(168, 85, 247, 0.2)',
                        borderColor: e.entityType === 'PARTNER' ? 'rgb(16, 185, 129)' :
                                    e.entityType === 'VENDOR' ? 'rgb(59, 130, 246)' :
                                    e.entityType === 'COMPETITOR' ? 'rgb(239, 68, 68)' :
                                    'rgb(168, 85, 247)'
                      }}
                    >
        ...
        <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
          ...
          {entities.length === 0 ? (
            <div className="text-center py-12">
              ...
              <button onClick={() => setShowAddEntity(true)} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 rounded-lg text-sm">
                + Add Custom Entity
              </button>
            </div>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {filteredEntities.map(e => (
                <div key={e.id} className="p-4 bg-slate-700/50 rounded-lg hover:bg-slate-700 transition-colors">
          ...
          {opportunities.length === 0 ? (
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {scoredOpportunities.map(o => (
                <div key={o.id} className="p-4 bg-slate-700/50 rounded-lg hover:bg-slate-700 transition-colors">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded flex items-center gap-1 ${
                        o.priority === 'HIGH' ? 'bg-red-500/20 text-red-300' :
                        o.priority === 'MEDIUM' ? 'bg-amber-500/20 text-amber-300' :
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 text-xs">
                      ...
                      {o.estimatedValue && (
                        <span className="text-purple-400">${(o.estimatedValue / 1000).toFixed(0)}K</span>
                      )}
                    </div>
      ...
      {showOpportunityExplainer && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={() => setShowOpportunityExplainer(false)}>
          <div className="bg-slate-800 rounded-xl p-6 w-full max-w-2xl border border-slate-700" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3 mb-6">
              {OPPORTUNITY_SIGNALS.map(signal => (
                <div key={signal.source} className="flex items-center gap-4 p-4 bg-slate-700/50 rounded-lg">
      ...
      {/* Import Modal */}
      {showImportModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={() => setShowImportModal(false)}>
