...
function Test-Endpoint {
    ...
    try {
        ...
        if ($response.$ExpectedField -eq $true -or $response.$ExpectedField) {
            ...
            Record-Evidence -TestName $Name -Category $Category -Status "passed" `
            ...
            Record-Evidence -TestName $Name -Category $Category -Status "failed" `
                -DurationMs $durationMs -Request $Body -Response $response `
...
$dataset = Test-Endpoint -Name "Generate dataset" -Method "POST" -Endpoint "/api/v1/sovereign-arch/rlhf/datasets" `
...
try {
    $completeResponse = Invoke-RestMethod -Uri "$BaseUrl/api/v1/evidence/ledger/suites/$SuiteId/complete" `
