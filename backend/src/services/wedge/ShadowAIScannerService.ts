...
// =============================================================================
// WEDGE 1: SHADOW AI SCANNER SERVICE — Enterprise Edition
// Detects unauthorized AI tool usage across an organization.
// Quantifies data leakage risk with dollar exposure estimates.
//
// Enterprise features:
//   - DB persistence via persistServiceRecord / loadServiceRecords
//   - Real data ingestion surface (endpoint agents, browser ext, SIEM, proxy)
//   - Dynamic compliance gap evaluation based on actual scan metrics
//   - Scan result persistence + restore on restart
// =============================================================================
...
class ShadowAIScannerService {
  ...
  async runScan(config: ScanConfig): Promise<ShadowAIScanResult> {
    ...
    const hasAuditTrail = orgRealDetections.length > 0;
    const complianceGaps = buildComplianceGaps(
      piiExposures, sourceCodeLeaks, unauthorizedTools.length,
      detections.length, hasAuditTrail, confidentialLeaks,
    );
