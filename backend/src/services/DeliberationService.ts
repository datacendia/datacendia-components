 ...
 *
 * @exports DeliberationService, deliberationService, AgentResponse, CrossExamination, Deliberation, ExecutiveSummary, DeliberationMinutes, MinuteEntry
 * @module services/DeliberationService
 */
...
export class DeliberationService extends BaseService {
  ...
  async saveDeliberation(deliberation: Omit<Deliberation, 'id' | 'createdAt'>): Promise<Deliberation> {
    const id = `delib-${Date.now()}-${crypto.randomUUID().slice(0, 9)}`;
...
IMPORTANT: Every array MUST have at least 2 items. Return ONLY the JSON, no other text.`;
  ...
  generatePDFReport(deliberation: Deliberation, summary: ExecutiveSummary): string {
    // PDF generation via browser print — for server-side PDF, install puppeteer on Enterprise deployments
...
<head>
  ...
  <style>
    ...
    h2 { color: #374151; margin-top: 30px; page-break-after: avoid; break-after: avoid-page; }
...
<body>
  <div class="no-print" style="color: #6b7280; font-size: 12px; margin-bottom: 12px;">
    Print/export this page from your browser for PDF output.
  </div>
  <h1>${summary.title}</h1>
  <div class="meta">
  ...
  <h2 class="page-break">Dissenting Views</h2>
  <ul>${summary.dissent.map(d => `<li>${d}</li>`).join('')}</ul>
  ` : ''}
  
  <hr class="no-print" style="margin-top: 40px;">
  <p class="no-print" style="color: #9ca3af; font-size: 12px;">
