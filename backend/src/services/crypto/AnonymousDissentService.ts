...
export class AnonymousDissentService {
  ...
  verifyDissent(dissent: AnonymousDissent): DissentVerification {
    ...
    try {
      ...
      if (n === 0 || sig.ringSize !== n || responses.length !== n) {
        ...
        // Bound to exactly what the signer bound: message, key image, ring. Any
        // change to the statement, severity, deliberation, ring membership or
        // key image alters this prefix and the chain will not close.
        const prefix = concatBytes(
          utf8ToBytes(LSAG_DOMAIN),
          message,
          keyImage.toBytes(),
        ...
        // what closes the ring; no other position can be forced to.
  ...
  /**
   * H_p(P) — map a ring member's public key to an independent generator. Used
   * for the key image, so the image binds to the signer's key without revealing
   * which ring position it came from.
   */
