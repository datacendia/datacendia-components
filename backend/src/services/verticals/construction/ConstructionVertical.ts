/**
 * Service — Construction Vertical
 *
 * Business logic service implementing platform capabilities.
 *
 * @exports ConstructionDataConnector, ConstructionKnowledgeBase, ConstructionComplianceMapper, CreditDecisionSchema, TradeApprovalSchema, AMLEscalationSchema, PortfolioRebalanceSchema, CreditAnalysisAgentPreset
 * @module services/verticals/construction/ConstructionVertical
 */
...
/**
 * Construction and Real Estate Vertical Implementation
 * 
...
export interface CreditDecision extends BaseDecision {
  ...
  inputs: {
    ...
    ConstructionStatements: { year: number; revenue: number; netIncome: number }[];
  };
  outcome: {
    approved: boolean;
...
export type ConstructionDecision = CreditDecision | TradeApproval | AMLEscalation | PortfolioRebalance;

// ============================================================================
// LAYER 1: Construction DATA CONNECTOR
// ============================================================================
...
export class ConstructionDataConnector extends DataConnector<TradingSystemData | CoreBankingData> {
...
// ============================================================================
// LAYER 2: Construction KNOWLEDGE BASE
// ============================================================================
...
export class ConstructionKnowledgeBase extends VerticalKnowledgeBase {
  ...
  async enforceProvenance(docId: string): Promise<{ valid: boolean; issues: string[] }> {
    ...
    const maxAge = 24 * 60 * 60 * 1000; // 24 hours for Construction data
    if (age > maxAge) {
...
// ============================================================================
// LAYER 3: Construction COMPLIANCE MAPPER
// ============================================================================
...
export class ConstructionComplianceMapper extends ComplianceMapper {
  ...
  private async evaluateControlStatus(decision: ConstructionDecision, control: ComplianceControl): Promise<ComplianceEvidence['status']> {
    const violation = await this.evaluateControl(decision, control);
    if (violation) {
  ...
  private generateEvidenceDescription(decision: ConstructionDecision, control: ComplianceControl, status: ComplianceEvidence['status']): string {
...
// ============================================================================
// LAYER 4: Construction DECISION SCHEMAS
// ============================================================================
...
// ============================================================================
// LAYER 5: Construction AGENT PRESETS
// ============================================================================
...
export class CreditAnalysisAgentPreset extends AgentPreset {
  ...
  readonly capabilities: AgentCapability[] = [
    ...
    { id: 'Construction-analysis', name: 'Construction Statement Analysis', description: 'Analyze Construction statements', requiredPermissions: ['read:Constructions'] },
  ...
  readonly workflow: WorkflowStep[] = [
    ...
    },
    {
      ...
      agentId: 'Construction-analysis-agent',
      requiredInputs: ['ConstructionStatements'],
...
// ============================================================================
// LAYER 6: Construction DEFENSIBLE OUTPUT
// ============================================================================

export class ConstructionDefensibleOutput extends DefensibleOutput<ConstructionDecision> {
  readonly verticalId = 'Construction';

  async toRegulatorPacket(decision: ConstructionDecision, frameworkId: string): Promise<RegulatorPacket> {
  ...
  async toCourtBundle(decision: ConstructionDecision, caseReference?: string): Promise<CourtBundle> {
  ...
  async toAuditTrail(decision: ConstructionDecision, events: unknown[]): Promise<AuditTrail> {
  ...
  private generateExecutiveSummary(decision: ConstructionDecision): string {
  ...
  private generateAuditTrailSummary(decision: ConstructionDecision): string[] {
    return [
  ...
  private generateFactualBackground(decision: ConstructionDecision): string {
  ...
  private generateHumanOversightStatement(decision: ConstructionDecision): string {
  ...
  private generateEvidenceChain(decision: ConstructionDecision): string[] {
    return [
...
export class ConstructionVerticalImplementation implements VerticalImplementation<ConstructionDecision> {
  ...
  readonly knowledgeBase: ConstructionKnowledgeBase;
  readonly complianceMapper: ConstructionComplianceMapper;
  readonly decisionSchemas: Map<string, DecisionSchema<ConstructionDecision>>;
  readonly agentPresets: Map<string, AgentPreset>;
  readonly defensibleOutput: ConstructionDefensibleOutput;

  constructor() {
    this.dataConnector = new ConstructionDataConnector();
    this.knowledgeBase = new ConstructionKnowledgeBase();
    this.complianceMapper = new ConstructionComplianceMapper();
    
    // All 4 decision schemas
    this.decisionSchemas = new Map();
    this.decisionSchemas.set('credit', new CreditDecisionSchema() as unknown as DecisionSchema<ConstructionDecision>);
    this.decisionSchemas.set('trade', new TradeApprovalSchema() as unknown as DecisionSchema<ConstructionDecision>);
    this.decisionSchemas.set('aml', new AMLEscalationSchema() as unknown as DecisionSchema<ConstructionDecision>);
    this.decisionSchemas.set('rebalance', new PortfolioRebalanceSchema() as unknown as DecisionSchema<ConstructionDecision>);
