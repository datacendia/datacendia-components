...
export class PDFGeneratorService {
  ...
  async generateRegulatorsReceipt(receipt: {
    ...
    participants: {
      agents: { name: string; role: string; responseCount: number; citationCount?: number; dissented: boolean; confidenceAvg?: number }[];
    ...
    const sections: PDFSection[] = [
      ...
      { type: 'paragraph', content: `Generated: ${receipt.generatedAt.toISOString()}` },
      ...
      // Participants
      { type: 'heading', content: 'PARTICIPANTS', level: 2 },
      {
        type: 'table',
        headers: ['Agent', 'Role', 'Responses', 'Dissented', 'Confidence'],
        rows: receipt.participants.agents.map(a => [
          a.name,
          a.role,
          a.responseCount.toString(),
          a.dissented ? 'YES' : 'No',
    ...
    sections.push(
      ...
      { type: 'heading', content: 'RETENTION & LEGAL', level: 2 },
      {
        ...
        rows: [
          ...
          ['Retain Until', receipt.retention.retentionUntil instanceof Date ? receipt.retention.retentionUntil.toISOString() : String(receipt.retention.retentionUntil)],
      ...
      { type: 'spacer' },
      { type: 'divider' },
      { type: 'paragraph', content: "This Regulator's Receipt is a cryptographically signed record of the decision-making process. The Merkle root and hashes provide tamper-evident proof of the deliberation contents. This document is designed to be court-admissible and regulator-ready." },
      { type: 'paragraph', content: `© ${new Date().getFullYear()} Datacendia, LLC. All rights reserved.` },
    );
    ...
    return this.generatePDF(sections, {
      ...
      footerText: 'Cryptographically signed. Tamper-evident. Court-admissible.',
      pdfaCompliant: true,
    });
  }
