 ...
 *
 * @exports opsAgentService
 * @module services/ops/OpsAgentService
 */
...
// =============================================================================
// OPS AGENT SERVICE — Enterprise Edition
// Practical, everyday AI agents that execute tasks (not deliberate).
//
// Enterprise tier — all 4 agent types:
...
//
// Enterprise features over community:
//   - All 4 agent types (community: 2)
//   - 10 concurrent tasks (community: 3)
//   - 32K prompt / 128K context (community: 8K / 16K)
//   - Integrity hashing on results (SHA-256)
//   - Task scheduling (cron-based)
//   - 2000 task history, 7-day TTL (community: 50, 3-day)
//   - Multi-org RBAC scoping
//   - Full telemetry + token tracking
//
...
import type { InferenceChatMessage } from '../inference/InferenceProvider.js';
...
// ---------------------------------------------------------------------------
// Agent definitions — Enterprise: all 4 agents
// ---------------------------------------------------------------------------
...
class OpsAgentService {
  ...
  async checkAvailability(): Promise<{ available: boolean; provider: string }> {
    ...
    return { available, provider: available ? ollama.type : 'none' };
  }
  ...
  private async executeTask(
    ...
    try {
      ...
      // Enterprise: use chatWithTelemetry for full token tracking
      const chatResult = await ollama.chatWithTelemetry(messages, {
      ...
      // Enterprise: integrity hash + full telemetry
      result.metadata = {
        ...result.metadata,
        ...
        provider: chatResult.telemetry.provider,
        model: chatResult.telemetry.model,
        promptTokens: chatResult.telemetry.promptTokens,
        completionTokens: chatResult.telemetry.completionTokens,
        totalTokens: chatResult.telemetry.totalTokens,
        inferenceDurationMs: chatResult.telemetry.durationMs,
      };
      ...
      logger.info(`[OpsAgent] Task ${task.id} (${task.agentType}) completed in ${task.durationMs}ms [${chatResult.telemetry.totalTokens || '?'} tokens]`);
    } catch (err) {
