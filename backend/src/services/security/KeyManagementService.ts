 ...
 *
 * @exports KeyManagementService, keyManagementService, KMSConfig, KeyMetadata, SignatureResult, EncryptionResult, DecryptionResult, KMSProvider
 * @module services/security/KeyManagementService
 */
...
export class KeyManagementService {
  ...
  private async signWithAwsKms(data: Buffer, keyId: string | undefined, timestamp: Date): Promise<SignatureResult> {
    ...
    try {
      ...
      logger.error(`AWS KMS signing failed: ${(error as Error).message}`);
  ...
  private async verifyWithAwsKms(data: Buffer, signature: string, keyId?: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error(`AWS KMS verify failed: ${(error as Error).message}`);
      return false;
    }
  }
  ...
  private async encryptWithAwsKms(data: Buffer, keyId?: string): Promise<EncryptionResult> {
    ...
    try {
      ...
      logger.error(`AWS KMS encrypt failed: ${(error as Error).message}`);
  ...
  private async decryptWithAwsKms(ciphertext: string, keyId?: string): Promise<DecryptionResult> {
    ...
    try {
      ...
      logger.error(`AWS KMS decrypt failed: ${(error as Error).message}`);
