// =============================================================================
// AnonymousDissentService — LSAG ring signature
// =============================================================================
// Ported from datacendia-core, where the same defect was found. verifyDissent()
// computed sG, cP, sH and cI for every ring member, discarded all of them, and
// returned true whenever the scalars were positive. The signing side could not
// be verified either: it drew random c_i/s_i for non-signers and closed with
// c_signer = total - sum(others), with no challenge chain, so a verifier had
// nothing to recompute.
//
// Both sides are now a proper LSAG. The negative cases carry the weight — the
// defect being fixed was a verifier that could not fail.
// =============================================================================
