...
router.post('/workloads', async (req: Request, res: Response): Promise<void> => {
  try {
    const { name, type, priority, estimatedDurationMinutes, estimatedEnergyWh, preferredRegions, maxDeferralHours } = schema_0.parse(req.body);
