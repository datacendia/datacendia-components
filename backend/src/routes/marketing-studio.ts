...
router.post('/video-script', async (req: Request, res: Response) => {
  try {
    const { topic, duration, targetAudience } = schema_0.parse(req.body);
...
router.post('/image-prompt', async (req: Request, res: Response) => {
  try {
    const { purpose, platform, style } = schema_1.parse(req.body);
...
router.post('/pitch-deck', async (req: Request, res: Response) => {
  try {
    const { audience, focus } = schema_2.parse(req.body);
...
router.post('/copy', async (req: Request, res: Response) => {
  try {
    const { type, topic, tone } = schema_3.parse(req.body);
...
router.post('/social-media-calendar', async (req: Request, res: Response) => {
  try {
    const { themes, platforms, postsPerWeek } = schema_4.parse(req.body);
