...
class LiveMonitor {
  ...
  async connectToDatabase(): Promise<boolean> {
    try {
      ...
      console.log(`${colors.yellow}Database not available, using simulation mode${colors.reset}`);
      return false;
    }
  }
  ...
  async run(): Promise<void> {
    console.log(`${colors.brightCyan}Starting Datacendia Live Monitor...${colors.reset}`);
    ...
    if (redisConnected) {
      ...
      console.log(`${colors.green}Connected to Redis - listening for real events${colors.reset}`);
    }
    ...
    process.on('SIGINT', async () => {
      ...
      clearScreen();
      console.log(`${colors.brightCyan}Monitor stopped.${colors.reset}`);
      process.exit(0);
    });
  }
}
